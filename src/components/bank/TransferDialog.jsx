...
import { formatCurrency, parseCurrency, formatCPF, formatAgency, formatAccount } from '@/lib/utils';
...
const TransferDialog = ({ isOpen, setIsOpen, onTransferSuccess }) => {
    ...
    const handleCheckRecipient = async (e) => {
        ...
        if (method === 'key') rpcParams.p_value1 = key;
            ...
            rpcParams.p_value2 = account.replace(/\D/g, '');
        }
                            ...
                            <form onSubmit={handleCheckRecipient} className="space-y-4 pt-4">
                                <Tabs value={method} onValueChange={setMethod} className="w-full">
                                    ...
                                    <TabsContent value="account" className="pt-2 grid grid-cols-2 gap-2">
                                        <Input value={agency} onChange={e => setAgency(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="Agência (4 dígitos)" maxLength={4} />
                                        <Input value={account} onChange={e => setAccount(formatAccount(e.target.value))} placeholder="Conta (8 dígitos)" maxLength={9} />
                                    </TabsContent>
                                </Tabs>
                                <div><Label htmlFor="amount">Valor</Label><Input id="amount" value={amount} onChange={(e) => setAmount(formatCurrency(e.target.value, true))} placeholder="R$ 0,00" required /></div>
                            ...
                            <div className="py-4 space-y-4">
                                <div className="p-4 rounded-lg bg-slate-800/50">
                                    ...
                                    <div className="text-sm text-gray-300 space-y-1">
                                        ...
                                        <p><strong>Agência:</strong> {formatAgency(recipientInfo.agency_number)} / <strong>Conta:</strong> {formatAccount(recipientInfo.account_number)}</p>
