...
import { ChevronLeft, ChevronRight, Scale, Users, Briefcase, Newspaper, Gavel } from 'lucide-react';
...
const TestimonialsSection = () => {
  ...
  const CurrentIcon = testimonials[testimonialIndex].icon;
        ...
        <div className="relative max-w-4xl mx-auto">
            ...
            <AnimatePresence initial={false} custom={direction}>
              ...
              if (swipe < -swipeConfidenceThreshold) {
                ...
                <div className="glass-effect rounded-2xl p-8 md:p-12 text-center border border-white/10">
                  <CurrentIcon className="w-12 h-12 text-blue-400 mx-auto mb-6" />
          ...
          <div className="text-center mt-6 flex flex-col items-center gap-4">
             <div className="flex justify-center gap-3">
                {testimonials.map((_, i) => <button key={i} onClick={() => setPage([i, i > testimonialIndex ? 1 : -1])} className={cn("w-3 h-3 rounded-full transition-all duration-300", i === testimonialIndex ? "bg-white scale-125" : "bg-gray-600 hover:bg-gray-400")} />)}
             </div>
