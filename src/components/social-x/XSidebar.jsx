...
const XSidebar = ({ onPostClick }) => {
    ...
    const navItems = [
        { name: 'Início', icon: Home, path: '/services/x' },
    ...
    return (
        <div className="sticky top-0 h-screen flex flex-col justify-between py-4 px-2">
            <div>
                ...
                <nav className="flex flex-col items-center xl:items-start gap-2">
                    {navItems.map(item => (
                        <NavLink key={item.name} to={item.path} end={item.path === '/services/x'}
            ...
            <div className="xl:hidden mt-auto">
                 <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="w-14 h-14 rounded-full hover:bg-slate-800/50">
                             <img src={user?.x_avatar_url || `https://api.dicebear.com/7.x/micah/svg?seed=${user?.username}`} alt="Avatar" className="w-10 h-10 rounded-full bg-slate-600" />
                        </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="w-64 mb-2 glass-effect border-slate-700" side="top">
